...
export class DragDropService {
  static TOKEN: InjectionKey<DragDropService> = Symbol('DRAG_DROP_SERVICE_TOKEN');
  dragData: any;
  draggedEl: any;
  ...
  dropTargets: Array<{ nativeElement: any }> = [];
  ...
  private _dragEmptyImage?: HTMLImageElement;
  get dragEmptyImage() {
    if (!this._dragEmptyImage) {
      this._dragEmptyImage = new Image();
      // safari的img必须要有src
      this._dragEmptyImage.src =
  ...
  enableDraggedCloneNodeFollowMouse() {
    if (!this.dragCloneNode) {
      ...
      [].forEach.call(targetCanvasArr, (canvas: HTMLCanvasElement, index: number) => {
  ...
  interceptChildNode(parentNode: Node, childNodeList: NodeListOf<Node>) {
  ...
  followMouse4CloneNode = (event: DragEvent) => {
  ...
  getBatchDragData(identity?: any, order: ((a: any, b: any) => number) | 'select' | 'draggedElFirst' = 'draggedElFirst') {
  ...
  public copyStyle(source: HTMLElement, target: HTMLElement) {
    ...
    for (let i = 0; i < computedStyle.length; i++) {
      const key = computedStyle[i] as any;
    ...
    for (let i = 0; i < source.children.length; i++) {
      this.copyStyle(source.children[i] as HTMLElement, target.children[i] as HTMLElement);
    }
  }
}

export function useDragDropService() {
