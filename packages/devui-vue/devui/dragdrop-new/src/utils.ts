...
export class Utils {
  ...
  public static slice<T>(args: T[], slice?: number, sliceEnd?: number): T[] {
    const ret: T[] = [];
    let len = args.length;
    ...
    const start = (slice as number) < 0 ? Math.max(0, slice! + len) : slice || 0;
...
export function injectFromContext<T>(token: InjectionKey<T> | string, context: any): T | undefined {
  return context[token as symbol | string];
}

export function getContext(): any {
  return (getCurrentInstance() as unknown as { provides: any }).provides;
}

export function provideToContext<T>(token: InjectionKey<T> | string, value: T, context: any) {
  context[token as symbol | string] = value;
}
