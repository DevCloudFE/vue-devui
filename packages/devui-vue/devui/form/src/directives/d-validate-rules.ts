...
import { EventBus, isObject, hasKey } from '../utils';
import './style.scss';
...
function handleValidateError({
  ...
  if (MessageShowTypeEnum.popover === messageShowType) {
    EventBus.emit('showPopoverErrorMessage', {
      ...
      popPosition,
      updateOn,
    } as ShowPopoverErrorMessageEventData);
    return;
  }
...
function validateFn({
  ...
  popPosition,
  updateOn,
}: Partial<ValidateFnParam>) {
  validator
    .validate({ modelName: modelValue })
    .then(() => {
...
export default {
  mounted(el: HTMLElement, binding: DirectiveBinding, vnode: VNode): void {
    ...
    // 获取指令绑定的值
    let { rules: bindingRules, options = {}, messageShowType = MessageShowTypeEnum.popover }: DirectiveBindingValue = binding.value;
    ...
    const isCustomValidator =
      customRule && isObject(customRule) && (hasKey(customRule, 'validators') || hasKey(customRule, 'asyncValidators'));
    ...
    // 处理表单提交验证
    formName &&
      EventBus.on(`formSubmit:${formName}`, () => {
