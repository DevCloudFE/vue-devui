...
export function useCodeEditor(props: CodeEditorProps, ctx: SetupContext) {
    ...
    function layoutOverlayWidget(lineNumber: number, ...positionInfos: PositionInfo[]): void {
        ...
        comments.value[index].domNode!.style.width = `${editorLayoutInfo.width - layoutInfo.minimapWidth! - layoutInfo.offsetLeft!}px`;
    ...
    function calculateLayoutInfo(positionInfos: PositionInfo[], editorLayoutInfo: any, index: number): LayoutInfo {
        let _offsetLeft = 0;
        const indexOffsetLeft = comments.value[index].offserLeft;
        if (indexOffsetLeft) {
    ...
    function handleDomNodePosition(top: number, hieght: number, index: number): void {
        comments.value[index].domNode!.style.height = `${hieght}px`;
